...
This is normaly done at compile time and not here, see the makefile
#define SAVEACCONLY*/
...
int simulation(int Tr1, char *filename, char *op="wt", int save_every1=1, double *xx=NULL, double *xxp=NULL) {
    ...
    if ((fptr = (fopen(filename, op)))) { 
		...
		if (save_every < 0)
			printf("twalk: Only every %d accepted iterations will be saved in file %s\n", abs(save_every), filename);
		else
			printf("twalk: Every %d iterations to be saved in file %s\n", save_every, filename);
	...
	delete mapx;
		...
		sec = time(NULL);
		printf("twalk: Finished, %4.1f%% of moved pars per it. ratio (%f/%d). Output in file %s,\n      %s\n",
...
void Remain(int Tr, int it, long sec1, long sec2) {
	...
	if (ax < 60) {
		...
		printf("Finish in approx. %ld seconds.\n", ax);
		fflush(stdout);
		return;
	}
	...
	if (ax <= 360) {
		...
		printf("Finish in approx. %ld minutes and %ld seconds.\n",
			ax/60, ax % 60);
		fflush(stdout);
		return;
	}
