...
//the "every" thining subsampling parameter is EVERY_MULT*All.Dim()
#define EVERY_MULT 5
//The burn in is BURN_IN_MULT*All.Dim()
#define BURN_IN_MULT 150

//Everey how many iterations we expect and acceptance: inverse of the acceptance rate
#define ACCEP_EV 20


main(int argc, char *argv[]) {// Command line: bacon inputfile outputfile  
...
*/
	...
	//ssize = (it/ACCEP_EV - BURN_IN_MULT*All.Dim())/(EVERY_MULT*All.Dim())
	//Then we let
	
	int it = ACCEP_EV * All.Dim() * (ssize * EVERY_MULT  +  BURN_IN_MULT);
