...
Input::Input(char *datafile, int emaxnumofcurves, int maxm) {
	...
	do
	{
		...
		if (strcmp( key, "Cal") == 0)
		{
			...
			if (strcmp( "IntCal09", line) == 0) {   //int bomb
				curves[numofcurves++] = new IntCal09((int) rpars[1]);

				continue;	
			}

			if (strcmp( "IntCal09Marine", line) == 0) {
				curves[numofcurves++] = new IntCal09Marine();

				continue;	
			}
			
			if (strcmp( "SHCal04", line) == 0) {
				if (numofpars == 0) //default, BombSH
					curves[numofcurves++] = new SHCal04();
				else
					curves[numofcurves++] = new SHCal04((int) rpars[1]);
		...
		if (strcmp( key, "Bacon") == 0)
		{
			...
			sscanf( pars[0], " %s", line); //type of object
			 ...
			 //	double MMinYr, double MMaxYr, double th0, double thp0, int uuseT, int more_pars=0)
			...
			if (strcmp( line, "MovNor") == 0)
					                //dets                K   H  hiatus_pars         a         b               
				bacon = new BaconMov( dets,  (int) rpars[1],  H, hiatus_pars, rpars[6], rpars[7],
					rpars[2], rpars[3], rpars[4], rpars[5], rpars[10], rpars[11], 0,      seed);
					//MinYr     MaxYr       th0      thp0         c0       cm   useNor
