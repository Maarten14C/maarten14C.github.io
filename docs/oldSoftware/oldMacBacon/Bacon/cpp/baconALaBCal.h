...
class BaconFix: public Bacon {
       protected:
			...
			int SetThetas(double *x) {
				...
				if (fcmp( theta[0], MinYr) == -1) // < MinYr
					WarnBeyondLimits++;
					//printf("bacon: WARNING: lower end (%f) below MinYr= %f\n", theta[0], MinYr);
					//beyond established limits limits
				for (int k=1; k<K; k++) {
				...
				if (fcmp( theta[K], MaxYr) == 1) 
					WarnBeyondLimits++;
					//printf("bacon: WARNING: upper end (%f) above MaxYr= %f\n", theta[K], MaxYr);
					//beyond established limits limits
       ...
       public:
	         BaconFix( Dets *detsdets, int KK, int HH, double **hiatus_pars, double aa, double bb,
				double MMinYr, double MMaxYr, double th0, double thp0, int uuseT, unsigned long int seed, int more_pars=0)
				...
				//The trabslation from x's to thetas's is done in method insupport
				theta = new double[K+1];

				//Set the sections, locations for the c's
				c0 = dets->d(0); 
				Dc = (dets->d(m-1)-c0)/(double) K; 
				...
				for (int k=0; k<H; k++) {
					if (fcmp( h[k], ((k == 0) ? c(K) :  h[k-1]) - Dc) != -1) { //we need only one per section
						printf("Bacon: ERROR: The hiatuses are not in decending order and/or less than %f\n", c(K));
				...
				if (H == 0) {  //with no hiatus
					for (int k=K-1; k>0; k--) { 
						x0[k]  = w0*x0[k+1] + (1.0-w0)*GammaSim( alpha[0], 1.0/beta[0]);					
						xp0[k] = wp0*xp0[k+1] + (1.0-wp0)*GammaSim( alpha[0], 1.0/beta[0]);
					}
				}
					...
					for (int k=K-1; k>0; k--) { 
						if ((fcmp( c(k-1), h[l]) == -1) && (fcmp( h[l], c(k)) != 1)) { //forgets
							...
							x0[k]  = w0*x0[k+1] + (1.0-w0)*GammaSim( alpha[l], 1.0/beta[l]);
					}
					...
					for (int k=K-1; k>0; k--) { 
						if ((fcmp( c(k-1), h[l]) == -1) && (fcmp( h[l], c(k)) != 1)) { //forgets
							...
							xp0[k]  = wp0*xp0[k+1] + (1.0-wp0)*GammaSim( alpha[l], 1.0/beta[l]);
					}
			...
			//x[0] is theta[0], then x[1] ... x[K], x[K+1]=w
	         int insupport(double *x) {
				...
				//************ THIS IS BACON A LA BCAL
				// Change this   |   to 0.0 to put it back to normal
				if (fcmp( x[K], 1.0) != 1) //acc. rate alpha_{K} <= 1, out of support
					return 0;
				if (fcmp( 1000.0, x[k]) != 1) //acc. 1000 <= rate alpha_{K}, out of support
					return 0;


				/**** NO FURTHER assumptions, no model on acc rates etc. only they need to be positive,
				***** therefore the thetas are ordered 
				if (H == 0) {
					//printf("A: %d  %f  %d\n", 0, x[0], K); 
					for (int k=1; k<K; k++) {
			...
			void PrintNumWarnings() { 
				if (WarnBeyondLimits != 0) {
					...
					printf("bacon: WARNING: calibration attempeted beyond MinYr= %f or MaxYr= %f\n", MinYr, MaxYr);
				}
			}
			...
			virtual double eval(double *x, int prime) {
				...
				//**** THIS IS BACON A LA BCAL, WE ONLY CARE NOW FOR THE THETAS ****
				...
				else { //uses standar normal model
					for (int j=0; j<(m-1); j++) {
				...
				/**** THE REST OF THE PRIORS ARE NO USED
				//printf(" Uli=%f", Uli);
...
//This one implements Bacon but with moving c's, is very similar to BaconFix
class BaconMov: public BaconFix {
       protected:

			double *brk;
			double c(int i) { //The depth divisions, now are parameters
				if (i == 0) {  return c0; }
				if (i == K) {  return c0+K*Dc; }
       ...
       public:
	         BaconMov( Dets *detsdets, int KK, int HH, double **hiatus_pars, double aa, double bb,
				double MMinYr, double MMaxYr, double th0, double thp0, int uuseT, unsigned long int seed): 
				BaconFix( detsdets, KK, HH, hiatus_pars, aa, bb, MMinYr, MMaxYr, th0, thp0, uuseT, seed, KK-1) {
				// Both use the same parameters, adding K-1 parameters at the end.

				brk = new double[K-1];

				//x[0] is theta[0], then x[1] ... x[K], x[K+1]=w,
				//then x[K+2+0] = brk[0], ... , x[K+2+K-2]= brk[K-2]
				//Set the brk's
				for (int k=0; k<(K-1); k++) {
					x0 [K+2+k]  = Unab( c0+k*Dc, c0+(k+1)*Dc);
					xp0[K+2+k]  = Unab( c0+k*Dc, c0+(k+1)*Dc);
				}

				//The BASE CLASS version of c(k) was used in the constructor of BaconFix then we need to
				//reset the accumulation rates using the new c's
				if (H == 0) { //initial values for the acc. rates, with no hiatus
					for (int k=0; k<(K-1); k++) {  brk[k] = x0[K+1+k]; }
					for (int k=0; k<(K-1); k++) {  brk[k] = xp0[K+1+k]; }
				}
				else {//initial values for the acc. rates, with hiatus
					...
					for (int k=K-1; k>0; k--) { 
						if ((fcmp( c(k-1), h[l]) == -1) && (fcmp( h[l], c(k)) != 1)) { //forgets
							x0[k]  = NorSim( ha[l]/Dc, sqrt(hb[l])/Dc);
							l++; //jump to next hiatus, but max one hiatuas in each section.
						}
						else //continue with  memory
							x0[k]  = w0*x0[k+1] + (1.0-w0)*GammaSim( alpha[l], 1.0/beta[l]);
					...
					for (int k=K-1; k>0; k--) { 
						if ((fcmp( c(k-1), h[l]) == -1) && (fcmp( h[l], c(k)) != 1)) { //forgets
							xp0[k]  = NorSim( ha[l]/Dc, sqrt(hb[l])/Dc);;
							l++; //jump to next hiatus, but max one hiatuas in each section.
						}
						else //continue with the memory
							xp0[k]  = wp0*xp0[k+1] + (1.0-wp0)*GammaSim( alpha[l], 1.0/beta[l]);
			 ...
			 double G( const double d, const double *x) {
			 
				int i;
				for (i=0; i<K; i++)
					if (fcmp( d, c(i+1)) == -1) break;
			...
			//then x[K+2+0] = brk[0], ... , x[K+2+K-2]= brk[K-2]
	         int insupport(double *x) {
				...
				for (int k=0; k<(K-1); k++) {
					brk[k] = x[K+2+k];		
					//printf("c(%d) = %f < c(%d) = %f\n", k, c(k), k+1, c(k+1));
					if (fcmp( c(k), c(k+1)) != -1) //out of support
									return 0; //we need: c0 < brk[0]=c(1) < ... < brk[K-3]=c(K-2) < brk[K-2]=c(K-1) 
				}
				//printf("c(%d) = %f < c(%d) = %f\n", K-1, c(K-1), K, c(K));
				if (fcmp( c(K-1), c(K)) != -1) //out of support
							return 0; //and we need: brk[K-2]=c(K-1) < c(K)  					
			
				//Finish with the insupport checks as with fixed c's
				//This time, the DERIVED CLASS version of c(k) is used (thus the correct version)
				return BaconFix::insupport(x);
			...
			double GetcK() { return c(K); }
	         ...
	         void ShowDescrip() {
				printf("BaconMov: Bacon jumps model with moving c's.\n"); 
				printf("          K= %d, H= %d, dim= %d, Dc=%f, c(0)= %f, c(K)= %f\n",
					K, H, get_dim(), Dc, c(0), c(K));
			}
			...
			double eval(double *x, int prime) {
				...
				//do the standard evalution as with fixed c's
				//This time, the DERIVED CLASS version of c(k) is used (thus the correct version)
				//Since the prior for the c's is constant on c1 < c2 < ...
				//we do not need to add anything, just check the support
				BaconFix::eval( x, prime);

				//The results are saved in Uprior and Uli
				//a prior for the c's could be added here
