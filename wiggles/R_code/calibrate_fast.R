...
if(!dir.exists("calibfast.pngs"))
  dir.create("calibfast.pngs")

xseq <- c(seq(xmin, xmax, length=1000))

for(i in 1:length(xseq)) {
  png(paste0("calibfast.pngs/img_", 1e5+i, ".png"), units="cm", res=300, height=15, width=15)
  draw.skeleton()
  done.points(i-1)
